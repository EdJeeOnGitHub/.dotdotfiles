...
#include takeup_transformed_data.stan
  ...
  if (use_county_effects) {
    reduced_beta_county_raw_ = multi_normal_rng(rep_vector(0, num_counties), identity_matrix(num_counties));
    reduced_beta_county_sd_ = normal_rng(0, reduced_beta_county_sd_sd);
  }
  
  if (use_age_groups) {
    reduced_beta_age_group_raw_ = multi_normal_rng(rep_vector(0, num_age_groups), identity_matrix(num_age_groups));
    reduced_beta_age_group_alpha_ = normal_rng(0, age_group_alpha_sd);
    
    if (use_age_group_gp) {
      reduced_beta_age_group_rho_ = normal_rng(0, age_group_rho_sd);
    }
  }
