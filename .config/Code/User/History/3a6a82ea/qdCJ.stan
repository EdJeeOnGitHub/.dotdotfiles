...
#include takeup_transformed_data.stan
  ...
  if (use_cluster_effects) {
    reduced_beta_cluster_raw_ = normal_rng(rep_vector(0.0, num_clusters), rep_vector(1.0, num_clusters));
  }
  
  reduced_beta_cluster_sd_ = normal_rng(0, reduced_beta_cluster_sd_sd);
  
  if (use_county_effects) {
    reduced_beta_county_raw_ = to_vector(std_normal_rng());
    reduced_beta_county_sd_ = normal_rng(0, reduced_beta_county_sd_sd);
  }
  
  if (use_age_groups) {
    reduced_beta_age_group_raw_ = to_vector(std_normal_rng());
