...
#include takeup_transformed_data.stan
  ...
  if (use_county_effects) {
    reduced_beta_county_raw_ = multi_normal_rng(rep(0, num_counties), identity_matrix(num_counties));
    reduced_beta_county_sd_ = normal_rng(0, reduced_beta_county_sd_sd);
  }
  
  if (use_age_groups) {
    reduced_beta_age_group_raw_ = multi_normal_rng(rep(0, num_age_groups), identity_matrix(num_age_groups));
