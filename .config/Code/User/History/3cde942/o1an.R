...
clean_df = subset_df %>%
    ...
    mutate(
        cens_event_time = if_else(
            death == 0,
            follow_up_time,
            event_time
        ),
        cens_et_days = interval(treatment_time, cens_event_time) %/% days(1), 
        right_interval = if_else(
            precision_evt == "month", 
            cens_et_days + 30, 
            cens_et_days + 1
        ),
        left_interval = cens_et_days
    ) %>%
    group_by(study) %>%
