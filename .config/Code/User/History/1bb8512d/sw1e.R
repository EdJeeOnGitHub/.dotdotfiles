...
sample_plan = function(df, n_charges, n_circles){
...
sim_function = function(n_charges, n_circles, prop_sample){
...
target_n_circles = 20
circles_per_charge = c(1, 2, 3, 4, 5)
...
sim_draws = future_pmap_dfr(
    ...
    ~N_sim_function(40, params = list(n_charges = ..1, n_circles = ..2, prop_sample = ..3)), 
    .progress = TRUE
)


sim_draws %>%
    ggplot(aes(x = n_charges, y = p.value)) +
    geom_point()

summ_sim_draws = sim_draws %>%
    group_by(n_circles, n_charges, prop_sample) %>%
    summarise(
...
sim_draws %>%
    lm(
        data = .,
        p.value ~ n_unique_circles   + n_unique_individuals
    ) %>%
    tidy()
...
sim_draws %>%
    lm(
        data = .,
        p.value ~ n_unique_charges   + n_unique_individuals
    ) %>%
    tidy()
