...
charge_prop_seq = seq(from = 0.05, to = 0.25, length.out = 10)
circle_prop_seq = seq(from = 0.05, to = 0.25, length.out = 10)
prop_sample = 0.1
...
sim_draws %>%
    lm(
        data = .,
        p.value ~ n_unique_circles + n_unique_charges  + n_unique_individuals
    ) %>%
    tidy()
