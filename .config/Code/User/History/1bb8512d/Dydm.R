...
N_sim_function = function(N, params) {
    sim_df = future_map_dfr(
        ...
        mutate(
            params_n_charges = params$n_charges, 
...
sim_draws = future_pmap_dfr(
    ...
    ~N_sim_function(100, params = list(n_charges = ..1, n_circles = ..2, prop_sample = ..3)), 
    .progress = TRUE,
    .options = furrr_options(seed = TRUE)
)
plan(sequential)
...
sim_draws %>%
    ggplot(aes(x = realised_n_circles_per_charge, y = p.value)) +
    geom_point(alpha = 0.2) +
    geom_point(
        data = sim_draws %>%
                    group_by(params_n_circles) %>%
                    summarise( 
                        mean_p = mean(p.value)
                    ),
        inherit.aes = FALSE, 
        aes(x = params_n_circles + 0.1, 
            y = mean_p, 
            colour = factor(params_n_circles)), 
        size = 3 
    )  +
    guides(colour = "none") +
    theme_bw()
...
summ_sim_draws = sim_draws %>%
    group_by(params_n_circles, params_n_charges, params_prop_sample) %>%
    summarise(
...
sim_draws %>%
    lm(
        data = .,
        p.value ~ params_n_charges   + n_unique_individuals
    ) %>%
    tidy()
