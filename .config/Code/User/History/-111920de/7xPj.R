...
if (is.na(town_vac_df$n_vaccinated) | is.na(town_vac_df$n_timely)) {
...
full_town_vac_df = merge(
    ...
    town_vac_df,
    all.x = TRUE, 
    by.x = c("dates",  "vaccine", "district", "town"), 
...
if (is.na(district_vac_df$n_vaccinated) | is.na(district_vac_df$n_timely)) {
    stop("NAs in town vaccination numbers.")
}
