...
colnames(prep_karachi_east_df)

generate_correlated_fake_data = function(prep_data){
    lm_fit = prep_data %>%
        lmer( 
            data = .,
            sim_pr_mean ~ (1 | strat_id) + (1 | group_id) 
        )
    ...
    group_shock = rnorm( 
        ...
        sd = fitted_ranef[1, "sdcor"]
    )



karachi_fit
karachi_fit_ranef = ranef(karachi_fit)
karachi_fit_sd = VarCorr(karachi_fit) %>% as.data.frame()
charge_shock = rnorm(
    n = nrow(karachi_fit_ranef$charge_id), 
    mean = karachi_fit_ranef$charge_id[, 1], 
    sd = karachi_fit_sd[1, "sdcor"]
    )

names(charge_shock) = rownames(karachi_fit_ranef$charge_id)

subdivision_shock = rnorm(
    n = nrow(karachi_fit_ranef$subdivision_id), 
    mean = karachi_fit_ranef$subdivision_id[, 1],
    sd = karachi_fit_sd[2, "sdcor"]
)

names(subdivision_shock) = rownames(karachi_fit_ranef$subdivision_id)
subdivision_shock
