...
rural_kambar_param_df = expand.grid( 
    units_per_group = 1:5, 
    total_units_to_sample = 10:30 
) %>%
    mutate( 
        min_cutoff = 7*units_per_group
    ) %>%
    as_tibble() %>%
    filter(min_cutoff <= total_units_to_sample)

karachi_param_df = expand.grid( 
    units_per_group = 1:5, 
    total_units_to_sample = 10:30 
) %>%
    mutate( 
...
karachi_east_sim_draws = future_pmap_dfr(
    list( 
        karachi_param_df$units_per_group,
        karachi_param_df$total_units_to_sample
    ),
    ~N_sim_function(
        200, 
