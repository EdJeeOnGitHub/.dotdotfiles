...
```{r}
rural_df %>%
    filter(str_detect(district, "SUJA|KAMBAR")) %>%
    ggplot(aes( 
        x = pop, 
        fill = district
    )) +
    ...
    labs( 
        title = "Village Population by Taluka, Rural eSujaawal", 
        fill = "Taluka", 
...
rural_df %>%
    filter(str_detect(district, "SUJA")) %>%
    ggplot(aes( 
        x = pop, 
...
# ggsave("code/hh-power-sims/urban-se.png", width = 10, height = 10)
...
# ggsave("code/hh-power-sims/rural-se.png", width = 10, height = 10)
```
...
# ggsave("code/hh-power-sims/urban-no-se.png", width = 10, height = 10)
summ_sim_draws %>%
    filter(district_type == "rural") %>%
    # filter(!str_detect(district, "karachi")) %>%
    ggplot(aes( 
...
# ggsave("code/hh-power-sims/rural-no-se.png", width = 10, height = 10)
```
