...
```{r}
sim_draws = read_csv("data/output/hh-survey/sample-size-sim-draws.csv")
...
sim_draws %>%
    filter(total_units_to_sample == 30) %>%
    ggplot(aes( 
        x = n_unique_group_var, 
        y = std.error,
        colour = factor(total_units_to_sample)
...
x = rnorm(15, sd = 1)
y = rnorm(15, sd = sqrt(root_deff))


    good = lm(x ~ 1) %>%
        tidy() %>%
        mutate(type = "good", draw = draw) 
    bad = lm(y ~ 1) %>%
        tidy() %>%
        mutate(type = "bad") %>%
        mutate(draw = draw)
    return(bind_rows(good, bad))
}


ed = map_dfr(1:500, func)
ed %>%
    group_by( 
        draw
    ) %>%
    summarise( 
        ratio = (std.error[type == "bad"]^2)/(std.error[type == "good"]^2)
    ) %>%
    summarise( 
        mean(ratio)
    )
...
ed %>%
    ...
    ggplot(aes(x = type, y = mean_se, ymin = mean_se - 1.96*sd_se, ymax = mean_se + 1.96*sd_se)) +
    geom_pointrange()
...
func_n = function(draw){

x = rnorm(100, sd = 1)
y = rnorm(100, sd = sqrt(root_deff))
...
comp_df %>%
    ggplot(aes(
        x = factor(n),
     y = mean_se, 
     ymin = mean_se - 1.96*sd_se, ymax = mean_se + 1.96*sd_se, 
     colour = type)) +
    geom_pointrange(
