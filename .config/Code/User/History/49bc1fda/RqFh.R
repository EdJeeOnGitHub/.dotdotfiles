...
second_sim_df = read_csv("data/output/second-sim-results-202205201646.csv")
sim_df = read_csv("data/output/sim-results-202205201646.csv")
    
sim_df %>%
    select(N) %>%
    unique()
    second_sim_df %>%
        select(p_adjust_method)

# Adding simple Bonferroni-Holm adjustments
...
ggsave("data/output/plots/icc-ols-cluster-H1-H2-H4.png", width = 8, height = 6)
...
ggsave("data/output/plots/icc-ols-cluster-H3.png", width = 8, height = 6)




mh_summ_sim_df = sim_df %>%
    mutate(signif = as.numeric(adj_p < 0.05)) %>%
    generate_mde(
        warmup_draws = 1000, 
        N, J, n_clusters, rho
    )
mh_summ_sim_df
summ_sim_df
mh_second_summ_sim_df = second_sim_df %>%
    mutate(signif = as.numeric(adj_p < 0.05)) %>%
    generate_mde(
        warmup_draws = 1000, 
        N, J, n_clusters, rho
