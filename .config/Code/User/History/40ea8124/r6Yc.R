...
all_vaccines = vax_dt[, unique(vaccine)]
...
prep_vax_dt = prepare_data_for_did(
    vax_dt[vaccine %in% script_options$vaccine], 
...
if (nrow(prep_vax_dt) == 0) {
    stop("No observations present - are vaccines correctly specified in script_options?")
}
