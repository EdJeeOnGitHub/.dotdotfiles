...
script_options <- docopt::docopt(
  ...
  estimate-att-gt.R [ --fake-data --time-aggregation-level=<time-aggregation-level> --num-cores=<num-cores> --unit-aggregation-level=<unit-aggregation-level>]
  Options:
  ...
  --propensity-weight  NOT IMPLEMENTED YETjk
...
saveRDS(
    prep_town_vax_dt, 
    paste0("data/output/", fake_output_name, "town-", script_options$time_aggregation_level, "-prep-att-gt-data.csv")
)

saveRDS(
    prep_district_vax_dt, 
    paste0("data/output/", fake_output_name, "district-", script_options$time_aggregation_level, "-prep-att-gt-data.csv")
)
...
if (script_options$time_aggregation_level == "month") {
    ...
    iwalk(
        ...
        ~saveRDS(
            .x, 
            file = file.path("data/output/", paste0(fake_output_name, "district-", script_options$time_aggregation_level, "-", vaccines[.y], "-attgt.rds")) )
    )
}
