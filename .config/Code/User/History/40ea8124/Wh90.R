...
script_options <- docopt::docopt(
  stringr::str_glue("Usage:
    estimate-att-gt.R [options] [<vaccine>...]
    ...
    Options:
        ...
        --num-bootstrap-draws  Number of bootstrap draws to estimate ATT(g,t)s with. [default: 100] 
        --additional-districts  NOT IMPLEMENTED YET
        --propensity-weight  NOT IMPLEMENTED YET
"),
  args = if (interactive()) "--fake-data --time-aggregation-level=week --num-cores=6 penta1 dog" else commandArgs(trailingOnly = TRUE)
) 
...
if (length(script_options$vaccine) == 0) {
...
vaccines = script_options$vaccine 
missing_vaccines = setdiff(vaccines, vax_dt[, unique(vaccine)])
if (length(missing_vaccines) != 0) {
    stop(str_glue("The following vaccines: {missing_vaccines} are not present in dataset but supplied by user."))
}
