library(shiny)
source("shiny/arm-proportions.R")
...
server <- function(input, output) {
   ...
   dataInput = reactive({
        ...
        prop_df = enframe(prop$p_vector) %>%
            mutate("Treatment Arm" = c("1", "2", "4")) %>%
            mutate("Proportion" = paste0(round(value*100, digits = 0), "%")) %>%
            select(-value, -name)
        hyp_df = enframe(prop$mde_vector) %>%
            rename("Hypothesis MDE" = name, MDE = value) %>%
            mutate(MDE = paste0(round(MDE*100, digits = 2), "%"))
        return(list(
            hyp_df = hyp_df,
            prop_df = prop_df))
   })

    output$hyp_df = renderTable({
        dataInput()$hyp_df
    }, align = "cc")

    output$prop_df = renderTable({
        dataInput()$prop_df
    }, align = "cc")
