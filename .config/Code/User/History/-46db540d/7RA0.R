...
tidy_sim_output = function(sim_output) {
  ...
  if (!is.null(sim_output$bayes_fit)) {
    browser()
    # if not null then we have a matrix
    if (!is.null(dim(true_params$gamma)) {
    ...
    clean_true_gammas = as_tibble(true_params$gamma) %>%
      mutate(j = 1:n(), term = "gamma")  %>%
      rename(true_value = value) 
    clean_true_betas = as_tibble(true_params$beta) %>%
      mutate(j = 1:n(), term = "beta")  %>%
      rename(true_value = value) 
    ...
    comp_df = bind_rows(
      ...
      tidy_heckit     ) %>%
      select(term, j, estimate, conf.low, conf.high, model) %>%
      left_join(
        true_param_df, by = c("term", "j")
      )
  } else {
    comp_df = NULL
  }
