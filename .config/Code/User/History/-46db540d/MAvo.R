...
meta_gen_modelled_data = function(gen_modelled_data_args){
    ...
    recentre = gen_modelled_data_args$recentre
  ...
  if (selection_d < data$d) {
    ...
    h_split = lapply(splits, function(x){h[, x]})
    N_choices = length(h_split)
...
simulate_draw = function(seed = 1234,
  ...
  if (fit_model == TRUE) {
    if (sum(modelled_data$D) < data$p) {
      ...
      ols_h_fit = lm(modelled_data$y[modelled_data$D > 0] ~ 0 +  modelled_data$X[modelled_data$D > 0, ] +  modelled_data$h[modelled_data$D > 0, ])
...
tidy_sim_output = function(sim_output, type = "non-hierarchical") {
  ...
  if (type == "hierarchical") {
    ...
    tidy_freq_ols_h = sim_output$ols_h_fit %>%
      tidy(conf.int = TRUE) %>%
    ...
    if (type == "hierarchical") {
      comp_ols = bind_rows(
        ...
        tidy_freq_ols_h %>% mutate(model = "freq_ols_h")
      ) %>%
        select(term, j, k, estimate, conf.low, conf.high, model, std.error) %>%
        mutate(c = 1) %>%
        left_join(
