...
#include sec-model-priors-lkj.stan
    ...
    // y is measured with noise as it's in itself estimated
    {
        ...
        y_h = append_col(y_latent, h_a);
        mu_vec = append_col(Xb, Zg);

        for (i in 1:N_obs){
            y_obs[i] ~ normal(y_latent[i], y_latent_se_hat);
            target += multi_normal_cholesky_lpdf(y_h
            )
        }        
        ...
        for(i in 1:N) {
            if (D[i] == 1) {
                ...
                target += multi_normal_cholesky_lpdf(y_h[D_ind[i], ] | mu_vec[i, ], L);
            }
            if (D[i] == 0) {
                if (d == 1) {
