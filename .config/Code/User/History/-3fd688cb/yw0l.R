...
sim_param_file_name = paste0("sim-params-", format(now, "%Y%m%d%H%M-"), "id-", random_string,  ".csv")

second_sim_result_file_name = paste0("second-sim-results-", format(now, "%Y%m%d%H%M"),  ".csv")
second_sim_param_file_name = paste0("second-sim-params-", format(now, "%Y%m%d%H%M-"), "id-", random_string,  ".csv")


sim_params = script_options %>%
  as_tibble() %>%
  select(-contains("--")) %>%
  mutate(
    result_file = sim_result_file_name,
    param_file = sim_param_file_name
  )

dir.create("data/output/simulations", showWarnings = FALSE)
write_csv(simple_sim, paste0("data/output/simulations/", sim_result_file_name))
write_csv(sim_params, paste0("data/output/simulations/", sim_param_file_name))
