...
thompson_TE = function(signif_vec, TE_vec, batch = 5){
    N = length(signif_vec)
    df = tibble(
        signif = signif_vec,
        TE = TE_vec
...
run_sim = function(N, hyper_param_func){
    ...
    for (i in 1:N){
        next_TEs = thompson_TE(res_df$signif, res_df$TE)
...
sim_df %>%
    ...
    filter(signif > 0.8 & signif < 0.9) %>%
    select(N, rho, bin, signif, TE, n_bin) 
