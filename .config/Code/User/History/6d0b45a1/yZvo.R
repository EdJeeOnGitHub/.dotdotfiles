...
pr_assign_table = read_csv("data/output/incorrect-prop-score.csv")
pr_assign_table
...
sim_function = function(draw, data, vaccines, prop_score, yname = "n_vaxxed") {
...
add_pretrend_sim_prop = future_map_dfr(
    1:50,
    ~sim_function(
        draw = .x,
        vaccines = vaccines,
...
pretrend_sim_prop = future_map_dfr(
    1:50,
    ~sim_function(
        draw = .x,
        vaccines = vaccines,
...
sim_df = bind_rows(
    pretrend_sim %>% mutate(districts = "RCT", prop = FALSE),
    pretrend_sim_prop %>% mutate(districts = "RCT", prop = TRUE),
    add_pretrend_sim %>% mutate(districts = "Add", prop = FALSE),
    add_pretrend_sim_prop %>% mutate(districts = "Add", prop = TRUE)
)

sim_df %>%
    filter(districts == "RCT") %>% 
...
sim_df %>%
    filter(type == "dynamic")  %>%
    filter(districts == "Add") %>%
    filter(vaccine == "Penta-1") %>%
    filter( 
        draw == 4
    )   %>%
    ggplot(aes( 
...
sim_df %>%
    filter(type == "dynamic") %>%
    mutate(error = estimate - 0) %>%
    group_by(
        districts, 
        prop, 
        event.time
    ) %>%
    summarise(mean_error = median(abs(error))) %>%
    arrange(event.time)
