...
Options:
  ...
  args = if (interactive()) "sbc --num-sbc-draws=10 --iter=2000  --chains=1 --outputname=test --models=REDUCED_FORM_NO_RESTRICT --output-path=data/sbc_output_data --include-paths=stan_models --multilevel --test-run --save-mem --num-cores=2" else commandArgs(trailingOnly = TRUE)
) 
...
if (script_options$cmdstanr == TRUE) {
  ...
  rf_model = stan_model("stan_models/takeup_reduced.stan")
}
