...
if (script_options$test_run == TRUE) {
  ...
  sample_from_model = meta_sample_from_model(default_sample_from_model_args %>% list_modify(stan_model = sbc_model))
  ...
  sim_draw$bayes_fit$summary(variables = "ranks_", "mean")
  rank_draws = sim_draw$bayes_fit %>%
    gather_draws(ranks_[k]) 
  rank_draws %>%
  ...
  anon_f = function(seed){
    ...
    rank_draws = sim_draw$bayes_fit$summary(variables = "ranks_", "mean")
