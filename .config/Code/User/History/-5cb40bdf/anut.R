...
meta_gen_params = function(dgp = "rweibull"){
    function(seed,
        ...
        if (dgp == "rgompertz") {
            beta_transformed = -1*beta
            shape = abs(rnorm(1, 5, 5)) 
        } else {
...
meta_gen_modeled_data_function = function(dgp = "rweibull"){
    ...
    if (dgp == "rgompertz") {
        dgp_f = function(x, y, z){VGAM::rgompertz(n = x, shape = y, scale = z)}
    }
    function(seed,
             data,
             params) {
...
anon_f = function(seed) {
    ...
    gen_data = meta_gen_data_function( mu_beta = rep(1, 3), J = 2)
    gen_modeled_data = meta_gen_modeled_data_function(dgp = "rgompertz")
    gen_params = meta_gen_params(dgp = "rgompertz")
    data = gen_data(seed)
    params = gen_params(seed,
                        data)
    modeled_data = gen_modeled_data(seed,
                                    data,
                                    params)
    return(list(params = params, modeled_data = modeled_data, data = data))
}

anon_f_data = anon_f(1)
anon_f_data$params
...
data %>%
    ggplot(aes(x = interval_right, group = factor(draw))) +
    geom_density()  +
    theme_minimal()


anon_f_data = anon_f(10)
anon_f_data$params
data
data %>%
    filter(draw == 2) %>%
    ggplot(aes(x = interval_right)) +
    geom_histogram() 

gomp_model = cmdstan_model("stan/test-hierarchical-gompertz.stan")
options(mc.cores = 4)
gomp_fit = gomp_model$sample(
    c(anon_f_data$data, anon_f_data$modeled_data)
)

gomp_fit %>%
    gather_draws(beta[j,k], shape) %>%
    median_qi() %>%
    filter(.variable == "shape" | k != 1)

anon_f_data$params

# data %>%
# write_csv("data/fake-sim-data.csv")
...
# corr_hier_model = cmdstan_model("stan/hierarchical-corr-weibull.stan")
...
sbc_draw(14, dgp = "rgompertz", stan_model = gomp_model, hyper_params_data = data_params)
sbc_draw(14, dgp = "rweibull", stan_model = corr_hier_model, hyper_params_data = data_params)
...
hier_draws %>%
  ...
  guides(colour = "none")  +
  labs(
      title = "SBC - Hierarchical Gompertz - Shape parameter", 
      x = "Theoretical Quantile", 
      y = "Realised Quantile"
  )

# ggsave("data/plots/hier-gompertz-shape.png", width = 8, height = 6)
...
hier_draws %>%
  ...
  guides(colour = "none") +
  labs( 
      title = "SBC - Hierarchical Gompertz - beta_1, beta_2, and beta_3 across sites J_1-J_5",
      x = "Theoretical Quantile", 
      y = "Realised Quantile"
  )
# ggsave("data/plots/hier-gomp-beta.png", width = 12, height = 8)
################################################################################
###Simulation Time!!!!!!!!!!!###################################################
################################################################################
