...
if (use_county_effects) {
    for (k in 1:num_dist_group_treatments) {
        reduced_beta_county_raw_[, k] = to_vector(normal_rng(rep_array(0, num_counties), 1));
...
if (use_age_groups) {
    for (k in 1:num_dist_group_treatments) {
        reduced_beta_age_group_raw_[, k] = to_vector(normal_rng(rep_array(0, num_age_groups), 1));
