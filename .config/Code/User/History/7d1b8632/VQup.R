...
meta_gen_data_function = function(N = 1000, 
    ...
    function(seed) {
        ...
        return(list(
            ...
            randomisation_level = randomisation_level
        ))
    }
}
...
#' 
meta_gen_modeled_data_function = function(dgp = "logit", spillover_pct = 0.1){
    function(seed,
             data,
             params) {
        set.seed(seed + 3e6)
        ...
        rho = data$rho
        randomisation_level = data$randomisation_level
        if (dgp == "probit") {
            p_vec = pnorm(mu_vec)
        } 
        if (dgp == "logit") {
        ...
        if (dgp == "icc") {
            ...
            if (randomisation_level == "individual") {
                pi_cluster_df = cluster_df %>%
                    ungroup() %>%
                    mutate(treatment_arm = treatment_arm) %>%
                    group_by(cluster_id) %>%
                    mutate(pi_v = pmin(params$baseline_mean + TEs[treatment_arm], 1)) %>%
                    mutate(n_treated_in_c = sum(treatment_arm == 2),
                        n_in_c = n(), 
                        spill_pi = (n_treated_in_c/n_in_c)*spillover_pct*max(pi_v)) %>%
                    mutate(pi_no_spill = pi_v, 
                        pi_v = if_else(treatment_arm != 1, pi_v, pi_v + spill_pi))                        
                y_pi_cluster_df = pi_cluster_df %>%
                ungroup() %>%
                mutate(y = rbinom(n = data$N, size = 1, prob = pi_v))

                y_pi_cluster_df %>%
                    group_by(treatment_arm) %>%
                    summarise(pi_v = mean(pi_v), y = mean(y))
...
simulate_data = function(seed = 1234,
    ...
    gen_data = meta_gen_data_function(
        ...
        n_villages = hyper_params$n_villages,
        randomisation_level = hyper_params$randomisation_level 

    )
    gen_modeled_data = meta_gen_modeled_data_function(dgp = hyper_params$dgp, spillover_pct = hyper_params$spillover_pct)
