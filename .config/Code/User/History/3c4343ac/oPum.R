...
script_options %<>% 
  modify_at(c("chains", "iter", "threads", "num_mix_groups", "num_sbc_sims", "cores"), as.integer) %>% 
  modify_at(c("models"), ~ c(str_split(script_options$models, ",", simplify = TRUE)))
if (script_options$cmdstanr || script_options$beliefs || script_options$dist || script_options$sbc) {
  library(cmdstanr)
} else {
  library(rstan)
...
## Distance ----------------------------------------------------------------
  ...
  rank_fit = function(seed, model, stan_data, rank_var = "ranks_") {
    fit = model$sample(
      stan_data,
      chains = script_options$chains,
      seed = seed,
