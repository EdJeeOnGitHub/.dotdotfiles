...
script_options <- docopt::docopt(
  ...
  run_takeup.R sbc wtp [--chains=<chains> --cores=<cores>  --iter=<iter> --outputname=<output file name>  --include-paths=<paths> --output-path=<path> --num-sbc-sims=<num-sbc-sims> --strata]
  run_takeup.R sbc rf [--chains=<chains> --cores=<cores>  --iter=<iter> --outputname=<output file name>  --include-paths=<paths> --output-path=<path> --num-sbc-sims=<num-sbc-sims> --num-mix-groups=<num> --multilevel --age]
  run_takeup.R sbc beliefs [--chains=<chains> --cores=<cores>  --iter=<iter> --outputname=<output file name>  --include-paths=<paths> --output-path=<path> --num-sbc-sims=<num-sbc-sims> --num-mix-groups=<num> --multilevel --age]
  run_takeup.R sbc struct [--chains=<chains> --cores=<cores> --models=<models>  --iter=<iter> --outputname=<output file name>  --include-paths=<paths> --output-path=<path> --num-sbc-sims=<num-sbc-sims> --num-mix-groups=<num> --multilevel --age]
...
"),
  ...
  # args = if (interactive()) "sbc wtp --chains=1 --cores=6 --num-sbc-sims=100 --output-path=data/sbc_output_data --include-paths=stan_models --iter=2000  --outputname=test --strata" else commandArgs(trailingOnly = TRUE)
  # args = if (interactive()) "sbc beliefs --chains=1 --cores=1 --num-sbc-sims=1 --output-path=data/sbc_output_data --include-paths=stan_models --iter=2000  --outputname=test --multilevel --age" else commandArgs(trailingOnly = TRUE)
  ...
  args = if (interactive()) "sbc struct --num-mix-groups=1 --chains=1 --cores=1 --num-sbc-sims=1 --models=STRUCTURAL_LINEAR_U_SHOCKS --output-path=data/sbc_output_data --include-paths=stan_models --iter=2000  --outputname=test --multilevel " else commandArgs(trailingOnly = TRUE)
...
script_options %<>% 
  modify_at(c("chains", "iter", "threads", "num_mix_groups", "num_sbc_sims", "parallel_folds", "cores"), as.integer) %>% 
  modify_at(c("models"), ~ c(str_split(script_options$models, ",", simplify = TRUE)))

if (script_options$cmdstanr || script_options$beliefs || script_options$dist || script_options$wtp || script_options$sbc) {
  library(cmdstanr)
} else {
  library(rstan)
...
# Stan Run ----------------------------------------------------------------
## Takeup ------------------------------------------------------------------
if (script_options$takeup & !script_options$sbc) {
...
## Beliefs -----------------------------------------------------------------
} else if (script_options$beliefs & !script_options$sbc) {
...
## Distance ----------------------------------------------------------------
} else if (script_options$dist & !script_options$sbc) {
...
} else if (script_options$wtp & !script_options$sbc) {
...
### Results -----------------------------------------------------------------
...
} else if (script_options$sbc) { 
  rank_fit = function(seed, model, stan_data) {
    fit = model$sample(
      stan_data,
      chains = script_options$chains,
      seed = seed,
      iter_warmup = round(script_options$iter/2),
      iter_sampling = round(script_options$iter/2)
    )
    all_vars =  fit$metadata()$stan_variables
    rank_stat = fit$summary(all_vars[grep(".*ranks_", all_vars)], "mean") %>%
      mutate(seed = seed)
    return(rank_stat)
  }

  if (script_options$wtp) {
    sbc_model = cmdstan_model(file.path(script_options$include_paths, "wtp_model.stan"))
    sbc_stan_data = wtp_stan_data %>%
      list_modify(
        sbc = script_options$sbc,
        fit_wtp_model_to_data = TRUE,
        use_strata_levels = script_options$strata
      )
    sbc_output_name = str_c("wtp_", output_name, ".csv")
  }
  if (script_options$rf) {
    sbc_model = cmdstan_model(file.path(script_options$include_paths, "takeup_reduced.stan"))

    ## NOTE currently RF SBC is hardcoded for no restrictions
    sbc_stan_data = stan_data %>%
      map_at(c("cluster_treatment_map", "beliefs_ate_pairs"), ~ mutate(.x, across(.fns = as.integer)) %>% as.matrix()) %>%  # A tibble of factors no longer gets converted into an "array[,] int" in Stan.
    ...
    sbc_output_name = str_c("rf_", output_name, ".csv")

  }
  if (script_options$beliefs) {
    sbc_model = cmdstan_model(file.path(script_options$include_paths, "secobeliefs.stan"))
    sbc_stan_data = stan_data %>% 
      map_at(c("cluster_treatment_map", 
                "beliefs_ate_pairs"), 
            ~ mutate(.x, across(.fns = as.integer)) %>% as.matrix()) %>%  # A tibble of factors no longer gets converted into an "array[,] int" in Stan.
      discard(~ any(is.na(.x))) %>%
      list_modify(num_dist_group_mix = 1)

    sbc_output_name = str_c("beliefs_", output_name, ".csv")

  }

  if (script_options$struct) {
    stop()
    sbc_model = cmdstan_model(
      file.path(script_options$include_paths, "ed_temp_takeup_struct.stan"), 
    ...
    sbc_output_name = str_c("struct_", output_name, ".csv")

    fit = sbc_model$sample(
      sbc_stan_data,
      chains = script_options$chains,
      seed = 1,
      iter_warmup = round(script_options$iter/2),
  ...
  vals = c(-0.159237,-0.171288,-0.190005,-0.329729,-0.175108,-0.198844,-0.18277,-0.389312,-0.169362,-0.329886,-0.286526,-0.294506,-0.289119,-0.20133,-0.159751,-0.267686,-0.183383,-0.292617,-0.27612,-0.315106,-0.188419,-0.16949,-0.174146,-0.188326,-0.158899,-0.312884,-0.172348,-0.383144,-0.18534,-0.16825,-0.17776,-0.290959,-0.227034,-0.325322,-0.160904,-0.162294,-0.219651,-0.164563,-0.325206,-0.174764,-0.216768,-0.214666,-0.160538,-0.459829,-0.163863,-0.17183,-0.30516,-0.205458,-0.212321,-0.197593,-0.334136,-0.37013,-0.16297,-0.164956,-0.265769,-0.226038,-0.181687,-0.194183,-0.292624,-0.447657,-0.323525,-0.173106,-0.297855,-0.341122,-0.374468,-0.208501,-0.194837,-0.23805,-0.36256,-0.237605,-0.370139,-0.366559,-0.286617,-0.22706,-0.223031,-0.712831,-0.223764,-0.228585,-0.31209,-0.323652,-0.494528,-0.717818,-0.341376,-0.235502,-0.379376,-0.591702,-0.247016,-0.326198,-0.286944,-0.4393,-0.322263,-0.399854,-0.269921,-0.353786,-0.29933,-0.22255,-0.316045,-0.606202,-0.28394,-0.258466,-0.31741,-0.401628,-0.311955,-0.414346,-0.245592,-0.282135,-0.506128,-0.244388,-0.248859,-0.314536,-0.316775,-0.175663,-0.428318,-0.228593,-0.413542,-0.417006,-0.438624,-0.456679,-0.30711,-0.342801,-0.584115,-0.386467,-0.210806,-0.424092,-0.365113,-0.24406,-0.219783,-0.230919,-0.30393,-0.409169,-0.41052,-0.339521,-0.259749,-0.431765,-0.378246,-0.558488,-0.383158,-0.375463,-0.359814,-0.433248,-0.294928,-0.325205,-0.22902,-0.32197)
  total_error_sd = c(1.23803,1.23803,1.23803,1.23803)

  structural_cluster_takeup_prob_ = pnorm(- vals / total_error_sd)
  structural_cluster_takeup_prob_
  vals 
    ...
    fit$output()
      ...
      ed = rank_fit(
        seed = 1,
        model = sbc_model,
        stan_data = sbc_stan_data
        )

    plan(cluster, workers = script_options$cores)

    sbc_fits = future_map_dfr(
      1:script_options$num_sbc_sims,
      ~rank_fit(
        seed = .x,
        model = sbc_model,
        stan_data = sbc_stan_data
    ...
    sbc_fits %>%
      write_csv(file.path(script_options$output_path, sbc_output_name))
}  
