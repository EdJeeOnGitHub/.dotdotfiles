...
script_options <- docopt::docopt(
  ...
  run_takeup.R sbc wtp [--chains=<chains> --threads=<threads>  --iter=<iter> --outputname=<output file name>  --include-paths=<paths> --output-path=<path> --num-sbc-sims=<num-sbc-sims> --strata]
...
"),
  ...
  args = if (interactive()) "sbc wtp --chains=1 --num-sbc-sims=8 --output-path=data/sbc_output_data --include-paths=stan_models --iter=2000  --outputname=wtp_test --strata" else commandArgs(trailingOnly = TRUE)
...
## Distance ----------------------------------------------------------------
  ...
  if (script_options$wtp) {
    ...
    plan(cluster, workers = script_options$threads)
