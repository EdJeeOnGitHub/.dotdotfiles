...
"),
  ...
  args = if (interactive()) "sbc beliefs --chains=1 --cores=6 --num-sbc-sims=18 --output-path=data/sbc_output_data --include-paths=stan_models --iter=2000  --outputname=test --multilevel --age" else commandArgs(trailingOnly = TRUE)
...
## Distance ----------------------------------------------------------------
  ...
  rank_fit = function(seed, model, stan_data) {
    ...
    rank_stat = fit$summary(variables = "ranks_", "mean") %>%
      mutate(seed = seed)
    return(rank_stat)
  }
