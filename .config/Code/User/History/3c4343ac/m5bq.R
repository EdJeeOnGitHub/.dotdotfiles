...
## Distance ----------------------------------------------------------------
  ...
  if (script_options$wtp) {
    wtp_model = cmdstan_model(file.path(script_options$include_paths, "wtp_model.stan"))
    sbc_wtp_stan_data = wtp_stan_data %>%
      list_modify(
        sbc = script_options$sbc,
    ...
    sbc_fits = future_map_dfr(
      ...
      ~rank_fit(
        seed = .x,
        model = wtp_model,
        stan_data =sbc_wtp_stan_data
      ),
      .options = furrr_options(seed = TRUE),
      .progress = interactive()
    )
    sbc_fits %>%
      write_csv(file.path(script_options$output_path, str_c("wtp_", output_name, ".csv")))
  }
  if (script_options$rf) {
    stop()
    rf_model = cmdstan_model(file.path(script_options$include_paths, "takeup_reduced.stan"))
    sbc_rf_stan_data = wtp_stan_data %>%
      list_modify(
        sbc = script_options$sbc,
        fit_wtp_model_to_data = TRUE,
        use_strata_levels = script_options$strata
    ...
    dist_fit <- models %>% 
        stan_list(stan_data, script_options, use_cmdstanr = TRUE, include_paths = script_options$include_paths)
    plan(cluster, workers = script_options$cores)

      mod_stan_data = stan_data %>%
        map_at(c("cluster_treatment_map", "beliefs_ate_pairs"), ~ mutate(.x, across(.fns = as.integer)) %>% as.matrix()) %>%  # A tibble of factors no longer gets converted into an "array[,] int" in Stan.
    ...
    sbc_fits = map_dfr(
      ...
      ~rank_fit(
        seed = .x,
        model = rf_model,
        stan_data = stan_data %>% c(list(models$REDUCED_FORM_NO_RESTRICT))
      ),
      .options = furrr_options(seed = TRUE),
      .progress = interactive()
    )
