...
default_stan_model = cmdstan_model("stan/vec-selection-mvn.stan")
# default_stan_model = stan_model("stan/selection-mvn.stan")
heckit_model = stan_model("stan/vec-selection-mvn.stan")

default_data_args = list(N = 1000, p = 2, q = 2, d = 1, cutoff_quantile = NA)
default_params_args = list(mu_beta = 0, sd_beta = 1)
default_modelled_data_args = list(selection_type = "conventional", rubin = FALSE, selection_d = 1)
...
mod_fit = default_stan_model$sample(
  c(first_draw$data, first_draw$modelled_data) %>%
    list_modify(cutoff_quantile = 0.1, y_latent_se_hat = 1)
)
...
D_obs = map(simulated_draws, "N_observed") %>% unlist()
#### Plots #####
sbc_draws %>%
  group_by(term, j, model) %>%
  summarise(
    cal_50 = mean(between(rank_stat, 0.25, 0.75)),
    cal_95 = mean(between(rank_stat, 0.025, 0.975))
  )
...
test_that("Selection MVN Model Recovers Parameters", {
...
ggsave("simple-sbc.png", width = 8, height = 6)
