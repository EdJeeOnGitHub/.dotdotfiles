...
default_data_args = list(N = 100, p = 2, q = 2, cutoff_quantile = 0.5)
default_params_args = list(mu_beta = 1, sd_beta = 3)
...
tidy_sim_draws %>%
  filter(term == "beta") %>%
  group_by(model, term) %>%
  summarise(mean_err = mean(true_value - estimate), med_err = median(true_value - estimate))
