...
# default_stan_model = stan_model("stan/rw-heckit.stan")

default_data_args = list(N = 200, p = 2, q = 2, cutoff_quantile = NA)
default_params_args = list(mu_beta = 0, sd_beta = 5)
...
sbc_draws %>%
  ...
  summarise(
    ...
    cal_90 = mean(between(rank_stat, 0.025, 0.975))
  )
