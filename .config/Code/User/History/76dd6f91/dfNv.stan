...
// array[num_clusters, num_age_groups] int<lower = 0> sim_cluster_takeup_count; // not yet implemented
...
base_mu_rep_ = normal_lb_rng(0, mu_rep_sd, 0);

if (num_dist_param > 0) {
    for (ndp in 1:num_dist_param) {
        dist_beta_v_[ndp] = normal_rng(0, dist_beta_v_sd);
    }
...
if (use_cluster_effects) {
    for (nt in 1:num_treatments) {
        for (nc in 1:num_clusters) {
            structural_beta_cluster_raw_[nc, nt] = std_normal_rng();
        }
        structural_beta_cluster_sd_[nt] = normal_lb_rng(0, structural_beta_cluster_sd_sd, 0);
    }
}
...
if (use_county_effects) {
    for (nt in 1:num_treatments) {
        for (nc in 1:num_counties) {
            structural_beta_county_raw_[nc, nt] = std_normal_rng();
        }
        structural_beta_county_sd_[nt] = normal_lb_rng(0, structural_beta_county_sd_sd, 0);
    }
}

if (use_homoskedastic_shocks) {
    raw_u_sd_[1] = normal_lb_rng(0, 1, 0);
} else {
    for (nts in 1:num_treatment_shocks) {
        raw_u_sd_[nts] = normal_lb_rng(0, 1, 0);
    }
}
...
structural_treatment_effect_ = treatment_map_design_matrix * beta_;
...
if (fit_model_to_data) {
    if (suppress_reputation) {
        ...
        for (cluster_index in 1:num_clusters) {
            ...
            // print("OKOKOKOKOKOK \n");
            // print(structural_cluster_obs_v_[cluster_index]);
            // print("OKOKOKOKOKOK \n");
            // print("OKOKOKOKOKOK \n");
            // print("OKOKOKOKOKOK \n");
        }
    ...
    // print(structural_cluster_obs_v_);
    // print("above str obvs v, below, total error sd");
    // print(total_error_sd_);
...
if (fit_model_to_data) {
    array[num_obs] real tmp_takeup;
    // Take-up Likelihood 
    // if (use_binomial) {
        // reject("Not yet implemented");
        // Age groups not supported
        // sim_cluster_takeup_count[obs_cluster_id, 1] = binomial_rng(
        //     cluster_size[obs_cluster_id, 1], 
        //     structural_cluster_takeup_prob_[obs_cluster_id]);
    // } else {
        for (i in 1:num_obs) {
            sim_takeup[obs_cluster_id[i]] = bernoulli_rng(
                structural_cluster_takeup_prob_[obs_cluster_id[i]]
            );
        } 
        // tmp_takeup[obs_cluster_id] = bernoulli_rng(
        //     structural_cluster_takeup_prob_[obs_cluster_id]);
        // print(structural_cluster_takeup_prob_);
        // print(tmp_takeup);
        // sim_takeup = rep_array(3, num_obs);
    // }
}