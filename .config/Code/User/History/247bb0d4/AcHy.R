...
fit_1 = feols(
    fml = score_std ~ lag_score_std + hh_income + m_education | id_teacher^year + id_school^id_grade^id_class^id_cohort^year,
    data = raw_df
)


A_hat = predict_partial(
    fit_1,
    newdata = raw_df %>%
        select(-id_teacher)
)
clean_df = raw_df %>%
    filter(!is.na(lag_score_std)) %>%
    mutate( 
        A_hat = A_hat[, 1],
        v_ijt = score_std - A_hat
    )
...
estim_va_year = function(data, year) {
    ...
    reg_formula = as.formula(paste0(y_var, " ~  . -id_teacher"))
...
bias_df = clean_df %>%
    left_join(
        va_df,
        by = c("id_teacher", "year")
    ) 


bias_df %>%
    feols(
        fml = A_hat ~ value_added | year, 
        data = .
    )
