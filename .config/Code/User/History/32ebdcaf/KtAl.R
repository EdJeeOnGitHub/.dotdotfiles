...
clean_sim_result_df = left_join(
    ...
    mutate(
        ...
        sd_error = replace_na(sd_error, 1), 
        rank_data = replace_na(rank_data, FALSE)
    )
...
clean_sim_result_df %>%
    ...
    summarise(n_wt = n_distinct(weight_type))
...
clean_sim_result_df %>%
    ...
    summarise(pr_optimal = mean(true_optimal_arm == estimated_optimal_arm))
clean_sim_result_df %>%
...
clean_sim_result_df %>%
    filter(
        tau == 1, tau0 == 1, util_weight_sd == 0, sd_error == 5
    ) %>%
    group_by(
        draw
    ) %>%
    mutate( 
        rank = rank(mean_welfare)
    ) %>%
...
clean_sim_result_df %>%
    filter(
        tau == 1, tau0 == 1, util_weight_sd == 0, sd_error == 1
    ) %>%
    group_by(
        draw
    ) %>%
