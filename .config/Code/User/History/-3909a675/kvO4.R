...
if (script_options$fake_data) {
    town_vax_dt = town_vax_dt[date > "2021-06-01" & date < "2023-01-01"]
    district_vax_dt = district_vax_dt[date > "2021-06-01" & date < "2023-01-01"]
}
...
iwalk(
    ...
    ~ggsave( 
        plot = .x, 
        filename = paste0(
            "data/output/plots/", fake_output_name, "district-histogram-", 
            vaccines[.y], "-",
            script_options$time_aggregation_level,
            "-plot.png"), 
        width = 8, 
...
ggsave(
    plot = p_town_hist, 
    filename = paste0(
        "data/output/plots/", fake_output_name, "town-histogram-", 
...
plot_district_town_breakdown = function(data, vax){

    p = data %>%
        filter(vaccine == vax) %>%
        filter(date < ymd("2022-07-27")) %>%
        ggplot(aes( 
            x = date, 
            y = n_vaccinated,
            colour = town
