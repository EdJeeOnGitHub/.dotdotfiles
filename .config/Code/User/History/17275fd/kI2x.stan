...
transformed parameters {
  vector[K_pooled] theta_k_raw;
  vector[K_pooled] theta_k;
  real<lower=0> shape;
  vector[K] baseline_k_raw;
  vector[K] baseline_k;
  ...
  if(pooling_type == 0) {
    ...
    theta_k_raw = rep_vector(mu[1], K_pooled) + tau[1]*eta;
  }

  if(pooling_baseline == 0) {
    baseline_k_raw = eta_baseline;
  }
...
/*
Ed Notes:
eta is covariate parameter  
mu is constant parameter
tau is variance which we multiply eta by
...
*/
  ...
  if (N > 0) {
    for (i in 1:N) {
      //LOGIT/SURVIVAL?-specific:
      if(pooling_type < 2){
        for(i in 1:N) {
          scale[i] = exp(baseline_k[site[i]] + theta_k[site[i]] * treatment[i] + fe[i]);
          if (censoring[i] == 1) {
            target += weibull_lccdf(interval_left[i] | shape, scale[i]);
          } 
          else {
            target += log_diff_exp(
              weibull_lcdf(interval_right[i] | shape, scale[i]),
      ...
      if(pooling_type == 2) {
        for(i in 1:N)
          scale[i] = exp(baseline_k[site[i]] + mu[1] * treatment[i] + fe[i]);
          if (censoring[i] == 1) {
            target += weibull_lccdf(interval_left[i] | shape, scale[i]);
          } 
          else {
