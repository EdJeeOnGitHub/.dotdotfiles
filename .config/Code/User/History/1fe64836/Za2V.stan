...
obs_beta_common_sd_ = normal_lb_rng(0, 0.125, 0);

hyper_beta_1ord_[1] = std_normal_rng();
hyper_beta_1ord_[2:] = to_row_vector(normal_rng(rep_array(0, num_treatments - 1), 0.5));

hyper_dist_beta_1ord_[1] = std_normal_rng();
hyper_dist_beta_1ord_[2:] = to_row_vector(normal_rng(rep_array(0, num_treatments - 1), 0.5));
...
for (k in 1:num_treatments) {
  if (beliefs_use_cluster_level) {
    cluster_beta_1ord_raw_[, k] = to_vector(normal_rng(rep_array(0, num_clusters), 1)); 
    cluster_beta_1ord_sd_[k] = normal_lb_rng(0, 0.25, 0);

    cluster_dist_beta_1ord_raw_[, k] = to_vector(normal_rng(rep_array(0, num_clusters), 1));
    cluster_dist_beta_1ord_sd_[k] = normal_lb_rng(0, 0.25, 0);

    cluster_beta_2ord_raw_[, k] = to_vector(normal_rng(rep_array(0, num_clusters), 1)); 
    cluster_beta_2ord_sd_[k] = normal_lb_rng(0, 0.25, 0);

    cluster_dist_beta_2ord_raw_[, k] = to_vector(normal_rng(rep_array(0, num_clusters), 1));
    cluster_dist_beta_2ord_sd_[k] = normal_lb_rng(0, 0.25, 0);
  }

  if (beliefs_use_stratum_level) {
    stratum_beta_1ord_raw_[, k] = to_vector(normal_rng(rep_array(0, num_counties), 1));
    stratum_beta_1ord_sd_[k] = normal_lb_rng(0, 0.5, 0);
    
    stratum_dist_beta_1ord_raw_[, k] = to_vector(normal_rng(rep_array(0, num_counties), 1));
    stratum_dist_beta_1ord_sd_[k] = normal_lb_rng(0, 0.5, 0);

    stratum_beta_2ord_raw_[, k] = to_vector(normal_rng(rep_array(0, num_counties), 1));
    stratum_beta_2ord_sd_[k] = normal_lb_rng(0, 0.5, 0);
    
    stratum_dist_beta_2ord_raw_[, k] = to_vector(normal_rng(rep_array(0, num_counties), 1));
...
hyper_beta_2ord_[2:] = to_row_vector(normal_rng(rep_array(0, num_treatments - 1 ), 0.5));

hyper_dist_beta_2ord_[1] = std_normal_rng();
hyper_dist_beta_2ord_[2:] = to_row_vector(normal_rng(rep_array(0, num_treatments - 1), 0.5));
...
sim_num_knows_1ord = binomial_rng(
    num_recognized, 
    inv_logit(
      calculate_beliefs_latent_predictor(
        beliefs_treatment_design_matrix, 
        centered_obs_beta_1ord_, 
        centered_obs_dist_beta_1ord_, 
...
if (fit_beliefs_model_to_data) {
sim_num_knows_2ord = binomial_rng(
    num_recognized, 
    inv_logit(
      calculate_beliefs_latent_predictor(
        beliefs_treatment_design_matrix, 
        centered_obs_beta_2ord_, 
        centered_obs_dist_beta_2ord_, 
        cluster_standard_dist[beliefs_cluster_index])
      )
);
}
