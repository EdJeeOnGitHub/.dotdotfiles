...
rho_karachi_west_rural = rho %>% 
        filter(
            str_detect(district, "WEST") & 
            str_detect(place, "rural")) %>%
        ungroup() %>%
        select(rho) %>%
        pull()

rho_karachi_west_urban = rho %>% 
        filter(
            str_detect(district, "WEST") & 
            str_detect(place, "urban")) %>%
        ungroup() %>%
        select(rho) %>%
        pull()


rho_sujawal_rural = rho %>% 
        filter(
            str_detect(district, "SUJAWAL") & 
            str_detect(place, "rural")) %>%
        ungroup() %>%
        select(rho) %>%
        pull()

rho_sujawal_urban = rho %>% 
        filter(
            str_detect(district, "SUJAWAL") & 
            str_detect(place, "urban")) %>%
        ungroup() %>%
...
rural_CV_karachi_west_mde = map_dfr(
    seq(from = 1000, to = 5000, length.out = 100),
    ~
    tibble(
        mde = calculate_cv_mde(
            rho =  rho_karachi_west_rural,
...
urban_CV_karachi_west_mde = map_dfr(
    seq(from = 1000, to = 5000, length.out = 100),
    ~
    tibble(
        mde = calculate_cv_mde(
            rho = rho_karachi_west_urban,
            m = urban_mean_karachi_west_block_size,
            kappa = 0.8,
            alpha = 0.05,
            p_c = 0.5,
...
mde_df = bind_rows(
    urban_CV_karachi_west_mde %>% mutate(district = "Karachi West", type = "urban"),
    rural_CV_karachi_west_mde %>% mutate(district = "Karachi West", type = "rural"),
    urban_CV_sujawal_mde %>% mutate(district = "Sujawal", type = "urban"),
    rural_CV_sujawal_mde %>% mutate(district = "Sujawal", type = "rural")
) 
