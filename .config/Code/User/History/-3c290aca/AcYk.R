...
TS_draw = function(N,
    ...
    if (weight_type %in% c("estimated", "random assignment")) {
...
TS_simulation = function(N_rounds, beta, util_weights, K, N, J, weight_type = "estimated", sim_draw = 1) {
    ...
    for (i in 2:N_rounds) {
        if (weight_type != "random assignment") {
        ...
        if (weight_type == "random assignment") {
            # if random assignment we want to still update the optimal weights, 
            # we just don't want to use them in the treatment arm decision
            draw = TS_draw(
                N = N, 
                K = K, 
                J = J, 
