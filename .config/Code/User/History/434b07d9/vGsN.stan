...
if (fit_wtp_model_to_data) { 
  ...
  array[num_wtp_obs] int<lower=0,upper=1> sim_switch;
  array[num_wtp_obs] int<lower=-1,upper=1> sim_wtp_response;
  ...
  for (stratum_index in 1:num_strata) {
    ...
    for (wtp_obs_index in wtp_stratum_pos:wtp_stratum_end) {
      sim_gift_value[wtp_obs_index] = gumbel_rng(wtp_val_ + strata_gift_value[stratum_index]);
      // -2 so 1:3 => [-1, 0, 1]
      sim_gift_choice[wtp_obs_index] = which_max(sim_gift_value[wtp_obs_index]) - 2;
      sim_switch[wtp_obs_index] = sim_gift_value[(sim_gift_choice[wtp_obs_index] + 2)] - sim_gift_value[(-1*sim_gift_choice[wtp_obs_index] + 2)] < wtp_offer 
    }
