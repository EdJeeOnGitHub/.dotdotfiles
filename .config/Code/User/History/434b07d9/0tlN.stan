...
if (fit_wtp_model_to_data) { 
  ...
  row_vector[3] wtp_val_ = normal_rng(0, 1); // [BRACELET, NEITHER, CALENDAR]
  array[num_wtp_obs] row_vector[3] sim_gift_value;
  array[num_wtp_obs] int<lower=-1, upper=1> sim_gift_choice; 
  array[num_strata] row_vector[3] strata_gift_value;
  for (stratum_index in 1:num_strata) {
    ...
    real wtp_cal_strata_val_ = std_normal_rng();
    real wtp_bracelet_strata_val_ = std_normal_rng();
    real wtp_neither_strata_val_ = std_normal_rng();
    strata_gift_value[stratum_index] = normal_rng(strata_effect_wtp_mu_, strata_wtp_mu_tau_);

  // here ed

    for (wtp_obs_index in wtp_stratum_pos:wtp_stratum_end) {
      sim_gift_value[wtp_obs_index] = gumbel_rng(wtp_val_ + strata_wtp_mu_[strata_gift_value], 1);
