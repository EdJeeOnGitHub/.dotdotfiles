...
if (fit_wtp_model_to_data) { 
  ...
  for (stratum_index in 1:num_strata) {
    ...
    vector[3] strata_gift_value[stratum_index] = normal_rng(strata_effect_wtp_mu_, strata_wtp_mu_tau_);

  // here ed

    for (wtp_obs_index in wtp_stratum_pos:wtp_stratum_end) {
      // -2 so 1:3 => [-1, 0, 1]
      sim_gift_choice[wtp_obs_index] = categorical_logit_rng(wtp_val_ + strata_wtp_mu_[stratum_index]) - 2;
    }
  // here ed
    for (wtp_obs_index in wtp_stratum_pos:wtp_stratum_end) {
