  ...
  if (fit_model_to_data && !sbc) {
    ...
    if (sbc) {
        ...
        // not yet implemented
            ...
            sim_takeup[included_monitored_obs] ~ bernoulli(structural_cluster_takeup_prob[obs_cluster_id[included_monitored_obs]]);
        }
    }
...
#include beliefs_generated_quantities.stan
    ...
    array[num_beliefs_obs, num_treatments] int centered_obs_beta_2ord_ranks_;
    array[num_discrete_dist] ordered[num_dist_group_mix] group_dist_mean_ranks_;
    array[num_clusters, num_dist_group_treatments] int structural_cluster_benefit_ranks_;
    for (k in 1:num_beliefs_obs) {
        for (j in 1:num_treatments) {
            if (sbc) {
                ...
                centered_obs_beta_2ord_ranks_[k, j] = centered_obs_beta_2ord_[k, j] < centered_obs_beta_2ord[k, j];
            } else {
                centered_obs_beta_1ord_ranks_[k, j] = -1;
                centered_obs_beta_2ord_ranks_[k, j] = -1;
            }
        }
    }
  ...
  matrix[num_clusters, num_dist_group_treatments] structural_cluster_benefit_ = 
        rep_matrix(structural_treatment_effect_', num_clusters) + 
        (structural_beta_cluster_ + structural_beta_county_[cluster_county_id]) * treatment_map_design_matrix';


    for (k in 1:num_clusters) {
        for (j in 1:num_dist_group_treatments) {
            if (sbc) {
                structural_cluster_benefit_ranks_[k, j] = structural_beta_cluster_[k, j] < structural_beta_cluster[k, j];
            } else {
                structural_cluster_benefit_ranks_[k, j] =  -1;
            }
        }
    } 
