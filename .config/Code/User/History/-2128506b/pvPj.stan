...
if (fit_wtp_model_to_data) { 
    ...
    array[num_wtp_obs] valuation_difference; // [BRACELET, NEITHER, CALENDAR]
    ...
    sim_wtp_response = rep_vector(-1, num_wtp_obs); 
    for (stratum_index in 1:num_strata) {
      ...
      for (wtp_obs_index in wtp_stratum_pos:wtp_stratum_end) {
        ...
        if (valuation_difference[wtp_obs_index] < 0) {
            ...
            if (-valuation_difference[wtp_obs_index] < rescaled_wtp_offer[wtp_obs_index]) {
        ...
        if (valuation_difference[wtp_obs_index] < rescaled_wtp_offer) 
        ...
        tmp_wtp_response[wtp_obs_index] = (sim_gift_value[wtp_obs_index, (sim_gift_choice[wtp_obs_index] + 2)] - sim_gift_value[wtp_obs_index, (-1*sim_gift_choice[wtp_obs_index] + 2)]) < scaled_wtp_offer[wtp_obs_index];
        if (tmp_wtp_response[wtp_obs_index] == 1) {
          sim_wtp_response[wtp_obs_index] = 1;
        } else if (tmp_wtp_response[wtp_obs_index] == 0) {
          sim_wtp_response[wtp_obs_index] = -1;
        } else {
