...
if (fit_wtp_model_to_data) { 
    ...
    for (stratum_index in 1:num_strata) {
      ...
      for (wtp_obs_index in wtp_stratum_pos:wtp_stratum_end) {
        ...
        sim_wtp_response[wtp_obs_index] = (-1)^((sim_gift_value[wtp_obs_index, (sim_gift_choice[wtp_obs_index] + 2)] - sim_gift_value[(-1*sim_gift_choice[wtp_obs_index] + 2)] < wtp_offer[wtp_obs_index]) - 1);
      }
