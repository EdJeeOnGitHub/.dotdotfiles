...
births_df = cohort_vax_df %>%
    filter(birth_year == 2020) %>%
    select(district, birth_year, vax_year_mon, monthly_births) %>%
    filter(year(vax_year_mon) == 2020) %>%
    unique() %>%
    mutate(month  = month(vax_year_mon))  %>%
    group_by(district, month) %>%
    summarise(n_births = unique(monthly_births)) %>%
    group_by(district) %>%
    summarise(annual_births = sum(n_births))
    
births_df %>%    
    filter(district %in% realised_RCT_sample) %>%
    mutate(total_births = sum(annual_births))
