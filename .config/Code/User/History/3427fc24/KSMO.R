...
TS_draw = function(N,
    ...
    if (weight_type == "equal") {
        ...
        clean_weight_fit = tibble(
            j = rep(1:J, 1000),
            .draw = rep(1:1000, each = J),
            .value = rep(1, J*1000)
        )
...
################################
TS_simulation = function(N_rounds, beta, util_weights, K, N, J, weight_type = "estimated", sim_draw = 1) {
...
sim_util_weights = map(1:10, ~rnorm(J_outcomes))
sim_util_weights
sim_beta = map(1:10, ~matrix(rnorm(N_arms*J_outcomes), nrow = N_arms, ncol = J_outcomes))
library(furrr)
plan(multisession, workers = 6)
simple_sim_estimated_weights = future_map_dfr(
    1:10,
    ~TS_simulation(
        N_rounds, 
        util_weights = sim_util_weights[[.x]],
        beta = sim_beta[[.x]],
        N_arms,
        N_per_round,
        J_outcomes,
...
simple_sim_equal_weights = future_map_dfr(
    1:10,
    ~TS_simulation(
        N_rounds, 
        util_weights = sim_util_weights[[.x]],
...
simple_sim_first_weights = future_map_dfr(
    1:10,
    ~TS_simulation(
        N_rounds, 
        util_weights = sim_util_weights[[.x]],
...
simple_sim_estimated_weights %>% mutate(weights = "estimated"),
simple_sim_first_weights %>% mutate(weights = "first"),
simple_sim_equal_weights %>% mutate(weights = "equal"),
)
...
sim_df %>%
    group_by(weights) %>%
    summarise( 
        pr_correct = sum(true_optimal_arm == estimated_optimal_arm)/n(), 
        mean_welfare = mean(mean_welfare)
    )
sim_df %>%
    filter(draw == 1)
analyse_MAB_results(thompson_sim)
