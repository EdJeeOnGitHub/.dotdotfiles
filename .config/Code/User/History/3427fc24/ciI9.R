...
################################
TS_simulation = function(N_rounds, beta, util_weights, K, N, J, estimate_weights = TRUE) {
    initial_shares = rep(1/K, K) 
    # initial_draw = TS_draw(N, initial_shares, beta)
    ...
    for (i in 2:N_rounds) {
        ...
        weight_matrix[i,] = draw$posterior_medians %>%
            filter(term == "mrs") %>%
            select(estimate) %>%
            pull() 
        ...
        posterior_medians = bind_rows(posterior_medians, draw$posterior_medians %>% mutate(round = i))
    ...
    return(list(result_dt = result_dt, posterior_medians = posterior_medians))
}
N_rounds = 5
N_per_round = 10
...
create_weight_df = function(true_util_weights) {
    true_weight_df = enframe(true_util_weights) %>%
...
thompson_sim$posterior_medians %>%
    filter(term == "beta") %>%
    ggplot(aes(
        x = round, 
        ymin = conf.low, 
