...
\begin{frame}{Thompson Posterior\footnote{$i$ now indexes individuals, and $k = 1, ..., K$ treatment arms.}}
    ...
    \begin{itemize}
        \item Notice how $\mathbf{\theta}^{k.}, \alpha^k, \beta^k$ are independent across 
    treatment arms $\implies$ unstructured bandit problem.

        \item Why settle for conjugate priors in 2022?
    \item Bernoulli-Beta makes sense for cross-sectional $\{0,1\}$ draws, but do we 
    really think, the problem is stateless? Conditional on having a job at $t$, is the 
    probability of having a job at $t+1$ a coin flip?
    \end{itemize}
\end{frame}
%------------------------------------------------
