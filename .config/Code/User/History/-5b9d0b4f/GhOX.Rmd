...
# Load and clean data
```{r}
all_df = read.csv("data/individual_data.csv")
...
for (mod in 1:2) {  # Note: Weibull doesn't converge for 2 studies
  ...
  tidy_fits_hr[[mod]] = fit_list %>% 
    ...
    mutate(estimate = ifelse(mod == 1, exp(estimate_trt), 
      exp(-estimate_trt)^exp(estimate_shape)), 
      conf.low = ifelse(mod == 1, exp(conf.low_trt), 
      exp(-conf.low_trt)^exp(conf.low_shape)), 
      conf.high = ifelse(mod == 1, exp(conf.high_trt), 
      exp(-conf.high_trt)^exp(conf.high_shape)),
...
for (i in 1:2) {  # Gompertz (3) didn't work: "No chains finished successfully. Unable to retrieve the fit."
  ...
  stan_draws[[i]] = surv_fit %>% gather_draws(beta[site_id, k], hazard_ratio[site_id])
}
```
...
plot_est <- function(i) {
  ...
  compare_fits_hr = bind_rows(tidy_fits_hr[[i]], all_tidy_draws_hr[[i]]) %>%
    left_join(site_name, by = "site_id") 
...
all_tidy_draws_hr = map(stan_draws, tidy_stan_draws_hr)
comp_dfs = map2(tidy_fits, 
                all_tidy_draws, 
                ~bind_rows(.x, .y) %>%
                  left_join(site_name, by = "site_id"))
# adding names
...
comp_hr_dfs[[2]] %>%
  ...
  plot_function() +
  labs(
    title = "Wei Model - Hazard Rates"
  )


comp_plots = map(comp_dfs, plot_function)
comp_hr_plots = map(comp_dfs, plot_function)
