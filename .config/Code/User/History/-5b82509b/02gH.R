...
default_data_args = list(N = 60, p = 2, q = 2, d= 1, cutoff_quantile = 0.2)
...
calculate_bayes_coverage = function(bayes_draws, interval) {
  ...
  bayes_draws %>%
    group_by(term, j, model) %>%
    summarise(
      coverage = mean(c_low < rank_stat & rank_stat < c_hi))
}
...
ols_coverage = map_dfr(
  ...
  ~calculate_freq_rp(ols_draws = ols_draws, interval = .x)
)
ols_coverage
