...
# Write leave one out dataset to sensitivity/ folder, one csv for each loo
# because we are cave-people.
1:nrow(study_peto_fixed) %>%
  map(
    ~{
      filter(study_peto_fixed, 1:n() != .x) %>%
      select(or, std.error) %>%
      write_csv(paste0("publication_bias/Applications/water/sensitivity/or-loo-row-", .x, ".csv"), col_names = FALSE)
    ...
    study_peto_fixed %>%
      ...
      select(-trial_name) %>%
          write_csv(paste0("publication_bias/Applications/water/sensitivity/or-labels-loo-row-", .x, ".csv"))
    }
      )
