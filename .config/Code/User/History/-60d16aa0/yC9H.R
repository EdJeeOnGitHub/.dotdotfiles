...
param_df = expand.grid( 
    units_per_group = 1:4, 
    total_units_to_sample = c(16, 20, 25, 30)
)
...
summ_sim_draws %>%
    ggplot(aes( 
        x = factor(total_units_to_sample), 
        y = mean_se,
    ...
    labs( 
        ...
        y = "Mean Standard Error",
        colour =  "N Blocks per Charge", 
        caption = "Lowest N set to 16 because 4 blocks per charge across 4 towns 
        gives 16 as lower limit where every town surveyed equal amount."
    )
