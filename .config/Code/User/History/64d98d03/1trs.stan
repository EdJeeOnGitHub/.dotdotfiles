...
#include sec-model-priors-lkj.stan
    ...
    to_vector(gamma) ~ normal(0, 5);
    {
        ...
        mu_vec_obs = append_col(X_obs, Z_obs) * append_col(beta, gamma)';
        ...
        // vector[N_obs] Xb_obs = to_vector(X_obs * beta);
        // vector[N_obs] Zg_obs = to_vector(Z_obs * gamma);
        // vector[N_obs] mu_vec_obs;
        // vector[N_unobs] Zg_unobs = to_vector(Z_unobs * gamma); // = to_array_1d(Z_unobs * gamma);
        // mu_vec_obs = append_col(Xb_obs, Zg_obs);
        // profile("i for loops") {

        //     for (i in 1:N_obs) {
        //         mu_vec_obs[i] = append_col(Xb_obs[i], Zg_obs[i, ]);
        //     }
        ...
        profile("modelling") {
            ...
            if (d == 1) {
                target += normal_lpdf(h_unobs[, 1] | Zg_unobs[, 1], sqrt(Sigma[2,2]));
