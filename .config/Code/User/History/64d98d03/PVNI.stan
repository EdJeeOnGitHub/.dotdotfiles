...
#include sec-model-priors-lkj.stan
    ...
    {
        // log-likelihood
        array[N_obs] real Xb_obs = to_array_1d(X_obs * beta);
        array[N_obs] row_vector[d] Zg_obs = to_array_2d(Z_obs * gamma);
        array[N_obs] row_vector[d+1] mu_vec_obs;
        array[N_unobs] row_vector[d] Zg_unobs = to_array_2d(Z_unobs * gamma); // = to_array_1d(Z_unobs * gamma);
