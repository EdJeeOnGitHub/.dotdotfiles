...
default_stan_model = stan_model("stan/quantile-simple-selection-mvn.stan")

default_data_args = list(N = 120, p = 2, q = 2, cutoff_quantile = 0.25)
default_params_args = list(mu_beta = 0, sd_beta = 1)
default_modelled_data_args = list(selection_type = "quantile")
...
calibration = function(stan_fit, params_true){
  stan_fit = first_draw$bayes_fit
  params_true = first_draw$params$beta
  ##
  params = c(
    "beta[1]",
    "beta[2]"
...
low_quantile = (stan_fit %>%
  summary())$summary[params, "25%"]

high_quantile = (stan_fit %>%
  summary())$summary[params, "75%"]
  ...
  unlist()
  names(coverage) = c("beta_1", "beta_2")
  return(coverage)
}
    ...
    ~{sim_draw = simulate_draw(
      ...
      N_observed = nobs(sim_draw$ols_fit)
       tidy_output = sim_draw %>%
        tidy_sim_output() %>%
        mutate(draw = .x)
        calibration_res = calibration(sim_draw$bayes_fit, sim_draw$params$beta)
...
sbc_draws %>%
  group_by(term, j) %>%
  summarise(cal_50 = mean(0.25 < rank_stat & rank_stat < 0.75))
