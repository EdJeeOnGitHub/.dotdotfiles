...
if (num_dist_group_mix > 1) {
  reject("Only implemented for num_dist_group_mix = 1.");
}
for (k in 1:num_discrete_dist) {
  group_dist_mix_[k, :] = rep_vector(1, num_dist_group_mix );
}
  ...
  if (fit_dist_model_to_data) {
    if (lognormal_dist_model) {
      sim_cluster_standard_dist[cluster_index] = lognormal_rng(
        group_dist_mean_[curr_assigned_dist_group, 1] 
        + county_dist_effect_[curr_assigned_dist_group, cluster_county_id[cluster_index]]
        + cluster_dist_effect_[curr_assigned_dist_group, cluster_index],
        group_dist_sd_[curr_assigned_dist_group, 1]
      );
    } else {
