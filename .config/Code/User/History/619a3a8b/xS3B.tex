...
Let's be more concrete. Suppose we observe five vaccination metrics, shown in Table \ref{tab:metrics},
across three regions, and only the two lowest regions are considered for the 
experimental sample:

% Please add the following required packages to your document preamble:
% \usepackage{booktabs}
\begin{table}[htb]
\caption{Vaccination Metrics}
\begin{tabular}{@{}|l|ccccc|@{}}
\toprule
Region   & Metric 1 & Metric 2 & Metric 3 & Metric 4 & Metric 5 \\ \midrule
Region $A$ & $\mathbf{3}$        &          8        & $\mathbf{7}$        & $\mathbf{3}$        & 2        \\
Region $B$ &          5          & $\mathbf{5}$        & $\mathbf{10}$       & $\mathbf{2}$        & $\mathbf{0}$        \\
Region $C$ & $\mathbf{4}$       & $\mathbf{3}$        &          11       &            5        & $\mathbf{1}$        \\ \bottomrule
\end{tabular}
\label{tab:metrics}
\end{table}


This gives us hypothetical experimental samples: 
...
Aggregating across the counterfactual draws by calculating the expected probability 
of being in an experimental sample gives us Table \ref{tab:mu}:
\begin{table}[htbp]
    \centering
    \caption{Probability Region In  Experimental Sample}
    \begin{tabular}{|l|c|c|} 
    \toprule
    Region   & Pr(experimental) = $\mu_r$  & $\sigma^2_r$ \\ 
    \hline
    Region $A$ & 0.6 &  0.21                                 \\
    Region $B$ & 0.6  &    0.21                             \\
    Region $C$ & 0.8   &    0.24                            \\
...
In reality, regions $A, B$ are in our realised 
experimental sample - however this is relatively surprising since region $C$ is 
the most likely to be experimental. 
\end{document}